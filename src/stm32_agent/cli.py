...
from .tools import flash as flash_tools
from .tools import monitor as monitor_tools
from .tools import svd as svd_tools
...
def svd_fetch(
    ...
    svd_tools.run_fetch_svd(chip=chip, workspace=workspace or Path.cwd(), output=output, force=force)
...
def build(
    ...
    project = project_ops.merge_config(
        project_ops.load_config(config),
        workspace=workspace,
        build_dir=build_dir,
        generator=generator,
...
def monitor(
    ...
    project = project_ops.merge_config(project_ops.load_config(config), workspace=workspace)
...
def debug_start(
    ...
    debug_tools.start_debug_session(
        project,
        backend=backend,
        openocd_path=openocd_path,
...
def debug_stop(
    ...
    project = project_ops.merge_config(project_ops.load_config(config), workspace=workspace)
    debug_tools.stop_debug_session(project)
...
def debug_step(
    ...
    project = project_ops.merge_config(project_ops.load_config(config), workspace=workspace)
    debug_tools.step_debug_session(project, instruction=instruction)
...
def debug_continue(
    ...
    project = project_ops.merge_config(project_ops.load_config(config), workspace=workspace)
    debug_tools.continue_debug_session(project, address=address)
...
def debug_registers(
    ...
    project = project_ops.merge_config(project_ops.load_config(config), workspace=workspace)
    debug_tools.dump_registers(project)
...
def debug_backtrace(
    ...
    project = project_ops.merge_config(project_ops.load_config(config), workspace=workspace)
    debug_tools.dump_backtrace(project)
